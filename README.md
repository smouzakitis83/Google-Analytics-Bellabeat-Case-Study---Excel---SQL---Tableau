# Project Background
Bellabeat is a high tech company that manufacturers health focused smart products period. By 2016, Bella beat had opened offices around the world and launched multiple products. Their products became available through a growing number of online retailers in addition to their own e-commerce channel on their website. 
Insights and recommendations are provided on the following key areas:

* **Activity Trends Analysis:** Weekly activity patterns by comparing total active minutes and sedentary minutes, average BMI for each week, a 24 hour average of steps and calories Consumer weight log info.
*  **Product Level Performance:** A analysis of Cyclistics bike product lines.
*  **Advertisement Strategy Recommendations**
